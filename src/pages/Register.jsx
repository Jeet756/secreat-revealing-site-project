...
export default function Register({ onDone }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const d = await apiCall('POST', `${API_BASE}/register`, form);
      setType('success');
      setMsg(d?.success || 'Account created! You can now log in.');
      setDone(true);
    } catch (e) {
  ...
  return (
    <div className="auth-page">
      <div className="auth-header">
        ...
        <p className="auth-subtitle">Join the vault of secrets</p>
      </div>
      ...
      {!done ? (
        <>
          ...
          <div className="auth-actions">
            <button className="btn btn-primary auth-submit" disabled={loading} onClick={handleSubmit}>
              {loading ? (
        ...
        <div style={{ textAlign: 'center', paddingTop: 8 }}>
          <button className="btn btn-gold auth-submit" onClick={onDone}>
            🔑 Go to Login
          </button>
        </div>
      )}
